...
import { Building, ChevronRight, Clock, Code2, DollarSign, Filter, LayoutDashboard, LogOut, Mail, Upload, Users, Wrench, X, Fingerprint } from 'lucide-react';
...
const navLinks = [
  ...
  { href: '/admin/biometrico', label: 'Biometría', icon: Fingerprint },
        ...
        <div className={cn("flex h-24 items-center border-b border-white/5", isOpen ? "px-6" : "justify-center px-0")}>
          <Link href="/admin" className={cn("flex items-center", isOpen ? "gap-3" : "justify-center w-full")}>
            ...
            <div className={cn("transition-all duration-500 overflow-hidden", isOpen ? "w-auto opacity-100" : "w-0 opacity-0 md:hidden")}>
              <h1 className="text-sm font-serif tracking-tighter leading-none text-white whitespace-nowrap ml-2">La Pampa</h1>
              <p className="text-[10px] font-bold tracking-[0.2em] text-[#D4AF37] whitespace-nowrap uppercase ml-2">SANTUARIO RESIDENCIAL</p>
            </div>
          </Link>
          {isOpen && (
